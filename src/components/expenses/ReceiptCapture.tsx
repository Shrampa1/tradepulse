...
import { parseReceiptPhoto } from "@/lib/api";
...
// Same camera/picker flow as PhotoCapture, but hands the photo to
// parse-receipt (vendor/amount/description) instead of parse-line-items.
export function ReceiptCapture({ onParsed }: Props) {
  ...
  async function handlePicked(result: ImagePicker.ImagePickerResult) {
    ...
    try {
      ...
      const { expense } = await parseReceiptPhoto({
        imageBase64: asset.base64,
        imageMimeType: asset.mimeType ?? "image/jpeg",
      });
      onParsed(expense);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not analyze the receipt.");
    } finally {
  ...
  return (
    <View className="gap-3 rounded-2xl border border-border bg-surface p-5">
      ...
      {isProcessing && <Text className="text-center text-sm text-subtle">Scanning receipt…</Text>}
