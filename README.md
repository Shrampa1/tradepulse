...
```bash
supabase functions deploy parse-line-items send-estimate create-deposit-session stripe-webhook public-quote check-overdue-invoices
...
In the Stripe Dashboard, add a webhook endpoint pointing at your deployed `stripe-webhook` function URL, listening for `checkout.session.completed`, and copy its signing secret into `STRIPE_WEBHOOK_SECRET`.
...
- Payment happens on the public web page (`public-quote`), not inside the native app — the customer receiving a quote doesn't have TradePulse installed, so that page has to be a plain link that works in any mobile browser. The tradesperson's Stripe account itself isn't onboarded via Stripe Connect here (`profiles.stripe_account_id` is reserved for that); as built, deposits/invoices are collected directly, which is enough for a single-tenant test but would need Stripe Connect before onboarding multiple tradespeople with payouts to their own bank accounts.
