...
- `npm run typecheck` excludes `supabase/functions/` — those run on Deno (global `Deno.serve`/`Deno.env`, remote `https://esm.sh/...` imports, `.ts`-suffixed relative imports), which the app's Node-oriented `tsc` config can't resolve. Supabase's own tooling (`supabase functions deploy`, or `deno check` if you have the Deno CLI) is what actually typechecks that code.
